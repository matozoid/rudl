...
{
	for (int i=0;i<8;i++) { if (mapmappt[i]!=NULL) { free (mapmappt[i]); mapmappt[i] = NULL; } }
	mappt = NULL;
...
//	maparraypt = NULL;
	...
	for (int i=0;(maplpDDSTiles[i]!=NULL);i++)
		SDL_FreeSurface(maplpDDSTiles[i]);

	if(ParallaxFilename){
		delete[] ParallaxFilename;
		ParallaxFilename=NULL;
	}
}
...
//	ScreenObject = Screen;
	...
	// load the map
	int result = MapLoad( Filename /*, Screen->GetDD()*/ );
...
int SDLMappy::MapDecodeBGFX (void)
{
	...
	{
		case 8:
			printf("8 bit maps are not yet supported...\n");
			return -1;
...
// ---------------------------------------------------------------------------------
void SDLMappy::MapDrawBG (SDL_Surface * lpDDS )
{
	...
	for(i=0; i<lpDDS->h/mapblockheight+2;i++)
		...
		{	
			...
			if (((mapxo+j)<mapwidth) && ((mapyo+i)<mapheight)){
				if (numtile>=0){
					SDL_BlitSurface(maplpDDSTiles[numtile>>5], NULL, lpDDS, &TileDestRect);
				}else{
					...
					numanim >>= 5;
					SDL_BlitSurface(maplpDDSTiles[numanim], NULL, lpDDS, &TileDestRect);
				}
			}
		}
}
...
// ---------------------------------------------------------------------------------
void SDLMappy::MapDrawBGT (SDL_Surface * lpDDS )
{
	int			i, j, mapvclip, maphclip;
    int         mapxo, mapyo;
    int 		numtile, numanim;
...
void SDLMappy::DrawParallax (SDL_Surface * lpDDS )
{
	int			i, j, mapvclip, maphclip;
    int         mapxo, mapyo;
    int 		indentx, indenty;
